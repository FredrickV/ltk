...
void Killed (edict_t *targ, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
        ...
        targ->enemy = attacker;
		//RiEvEr 
		//Fred Added some more random stuff here 
		// Radio usasge for bots in team games
...
void M_ReactToDamage (edict_t *targ, edict_t *attacker)
{
        ...
        // if we are a good guy monster and our attacker is a player
        // or another good guy, do not get mad at them
        if (targ->spawnflags & SPAWNFLAG_GOODGUY)
        {
                if (attacker->client || (attacker->spawnflags & SPAWNFLAG_GOODGUY))
                        return;
        }
		...
		// Paril
		// Don't get mad if they are good/bad.
		if (((targ->spawnflags & SPAWNFLAG_GOODGUY) && (!attacker->spawnflags & SPAWNFLAG_GOODGUY)) || ((!targ->spawnflags & SPAWNFLAG_GOODGUY) && (attacker->spawnflags & SPAWNFLAG_GOODGUY)))
		{
			if (targ->enemy)
				if (targ->enemy->client)
					targ->oldenemy = targ->enemy;
...
/*int CheckLocationalDamage (edict_t *targ, edict_t *inflictor, edict_t *attacker, 
							  vec3_t dir, vec3_t point, vec3_t normal, int damage, int knockback, 
							  int mod, float z_rel, float from_top, float targ_maxs2, int bleeding, int instant_dam, 
							  vec3_t line, vec_t dist, int head_success, int damage_type, int height, gitem_t *item, 
							  int do_sparks)
{
	int result[2];
	...
	z_rel = 0;
		...
		result[0] = damage;
		result[1] = bleeding;
		result[2] = 0; // Just incase.
		return result;
}*/

/*int Locational_Damage_Check (edict_t *targ, edict_t *inflictor, edict_t *attacker, 
							  vec3_t dir, vec3_t point, vec3_t normal, int damage, int knockback, 
							  int mod, float z_rel, float from_top, float targ_maxs2, int bleeding, int instant_dam, 
							  vec3_t line, vec_t dist, int head_success, int damage_type, int height, gitem_t *item, 
							  int do_sparks)
{
	int result[2];
	//if (!targ->client)
		//CheckLocationalDamage_Monster (targ, inflictor, attacker, dir, point, normal,damage, knockback, mod, z_rel, from_Top, targ_maxs2, bleeding, instant_dam, line, dist, head_success, damage_type, height, item); // Monster
	//else
		result = CheckLocationalDamage (targ, inflictor, attacker, dir, point, normal,damage, knockback, mod, z_rel, from_top, targ_maxs2, bleeding, instant_dam, line, dist, head_success, damage_type, height, item, do_sparks); // Human
...
//		int result[2];
		...
		// Paril
		// Monsters deal like 90 damage per shot.
		// This should help balance per skill.
		if (attacker != targ && attacker != world && !(attacker->client))
		{
			switch (sk)
			{
			case 0: // Easy
		...
		// Paril: Clients attacking eachother
        if (targ->client && attacker->client)
        { 
                if (!((targ != attacker) && 
                      ((deathmatch->value && ((int)(dmflags->value) 
                        & (DF_MODELTEAMS | DF_SKINTEAMS))) || coop->value) &&                       (attacker && attacker->client && 
                      ...
                      ((int)(dmflags->value) & (DF_NO_FRIENDLY_FIRE)))))
                {
                        ...
                        { 
                                ...
                                //safe_cprintf(targ, PRINT_HIGH, "z_rel is %f\n leg: %f stomach: %f chest: %f\n", z_rel, LEG_DAMAGE, STOMACH_DAMAGE, CHEST_DAMAGE );
                                //safe_cprintf(targ, PRINT_HIGH, "point[2]: %f targ->s.origin[2]: %f height: %d\n", point[2], targ->s.origin[2], height );
                                //safe_cprintf(targ, PRINT_HIGH, "abs(trag->min[2]): %d targ_max[2] %d\n", (int)abs(targ->mins[2]), (int)targ_maxs2);
                                //safe_cprintf(attacker, PRINT_HIGH, "abs(trag->min[2]): %d targ_max[2] %d\n", (int)abs(targ->mins[2]), (int)targ_maxs2); 
                                //safe_cprintf(attacker, PRINT_HIGH, "abs(trag->min[0]): %d targ_max[0] %d\n", (int)abs(targ->mins[0]), (int)targ->maxs[0]); 
                                //safe_cprintf(attacker, PRINT_HIGH, "abs(trag->min[1]): %d targ_max[1] %d\n", (int)abs(targ->mins[1]), (int)targ->maxs[1]); 
                                ...
                                if ( head_success )
                                {
                                        ...
                                        damage = damage*1.8 + 1;
                                        safe_cprintf(targ, PRINT_HIGH, "Head damage\n"); 
                                        if (attacker->client) 
                                                safe_cprintf(attacker, PRINT_HIGH, "You hit %s in the head\n", targ->client->pers.netname); 
                                ...
                                { 
                                        damage = damage * .25; 
                                        safe_cprintf(targ, PRINT_HIGH, "Leg damage\n"); 
                                        if (attacker->client) 
                                                safe_cprintf(attacker, PRINT_HIGH, "You hit %s in the legs\n", targ->client->pers.netname); 
                                ...
                                { 
                                        damage = damage * .4; 
                                        safe_cprintf(targ, PRINT_HIGH, "Stomach damage\n"); 
                                        if (attacker->client) 
                                                safe_cprintf(attacker, PRINT_HIGH, "You hit %s in the stomach\n", targ->client->pers.netname); 
                                ...
                                {   
                                                ...
                                                && mod != MOD_SNIPER )
                                        {
                                                if (attacker->client)
                                                {
                                                        safe_cprintf(attacker, PRINT_HIGH, "%s has a Kevlar Vest - AIM FOR THE HEAD!\n",
                                                                targ->client->pers.netname);
                                                        safe_cprintf(targ, PRINT_HIGH, "Kevlar Vest absorbed most of %s's shot\n", 
                                                                attacker->client->pers.netname);
                                                                /*
                                                                if (IsFemale(targ))
                                                                safe_cprintf(attacker, PRINT_HIGH, "You bruised %s through her Kevlar Vest\n", targ->client->pers.netname);
                                                                else
                                                                safe_cprintf(attacker, PRINT_HIGH, "You bruised %s through his Kevlar Vest\n", targ->client->pers.netname);
                                                        */
                                                }
                                                ...
                                                && mod == MOD_SNIPER )
                                        {
                                                if ( attacker->client )
                                                {
                                                        safe_cprintf(attacker, PRINT_HIGH, "%s has a Kevlar Vest, too bad you have AP rounds...\n",
                                                                targ->client->pers.netname);
                                                        safe_cprintf(targ, PRINT_HIGH, "Kevlar Vest absorbed some of %s's AP sniper round\n",
                                                                attacker->client->pers.netname);
                                                }
                                                damage = damage * .325;
                                                damage_type = LOC_CDAM;
                                        ...
                                        {
                                                damage = damage * .65; 
                                                safe_cprintf(targ, PRINT_HIGH, "Chest damage\n"); 
                                                if (attacker->client) 
                                                        safe_cprintf(attacker, PRINT_HIGH, "You hit %s in the chest\n", targ->client->pers.netname); 
                                                damage_type = LOC_CDAM;
                                        }
		...
		// Paril: Target is monster, human attacker
		else if ((targ != world && attacker != world) && (!targ->client && attacker->client))
		{
                if (targ != attacker)
                {
					char *hitch = "a monster";
		...
		// Paril: Attacker is a monster, but human is recieving damage.
		else if ((targ != world && attacker != world) && (targ->client && !attacker->client))
		{
                if (targ != attacker)
                {
		...
		// Paril: Does 4x damage if you hit a monster in
		// the head when he doesn't see you. 5x if you have silencer
		if (!(dflags & DAMAGE_RADIUS) && (targ->svflags & SVF_MONSTER) && (attacker->client) && (!targ->enemy) && (targ->health > 0) && head_success)
		{
			gitem_t *silencer = FindItem (SIL_NAME);
			int mult = 4;
			
			if ( attacker->client->pers.inventory[ITEM_INDEX(FindItem(SIL_NAME))] )
				mult += 1;
...
//FB 6/3/99
        ...
        if (take)
                {               
						...
						if (targ->client)
						{
							...
							if ( instant_dam && (!(targ->flags & FL_GODMODE))) 
        ...
        if (client)
        {
			//RiEvEr LTK Radio Message
		/*	if( targ->is_bot && (take > 20) && teamplay->value)
			{
				RadioBroadcast(targ, 0, "im_hit");
				BOTUT_Cmd_Say_f( targ, "I'm hit!");
			}*/
			//R LTK
						...
						else if (!attacker->client && attacker != world)
						{
							client->attacker = attacker;
							client->attacker_mod = mod;
							client->push_timeout = 50;
