...
////////////////////////////////////////////////////////////////////////
/*
 * $Header: /Action/LTK/SRC/ACESRC/acebot_nodes.c,v 1.2 2007/03/09 04:13:30 Fredrick Exp $
 *
 * $Log: acebot_nodes.c,v $
 * Revision 1.2  2007/03/09 04:13:30  Fredrick
 * *** empty log message ***
 *
...
// Fred
#define ZIP // Disable this and LTK will work without zip format
...
//	if( debug_mode->value && !self->is_bot)
...
//	return; // commented out for now. uncommend to show nodes during debugging,
	...
	else if(nodes[node].type== NODE_SNIPE)
...
int ACEND_AddNode(edict_t *self, int type)
{
  ...
  //   nodes[numnodes].origin[2] += 32;
	...
	if(debug_mode->value)
	{
		...
		else if(nodes[numnodes].type == NODE_SNIPE)
			  debug_printf("Node added %d type: Snipe\n",numnodes);
...
#undef ZIP
	...
	sprintf(DummyBuffer,"%s",filename);
	strcat(DummyBuffer,"Zip.exe");
	strcat(Buffer,"dummy");
	fp = fopen(Buffer,"w");

	i += sprintf(filename + i, level.mapname);
	sprintf(ZipBuffer,"%s","UnCompress\n");
	strcat(ZipBuffer,level.mapname);
   strcat(ZipBuffer,".zip");
	
	i += sprintf(filename + i, ".ltk");

	// write to dummy bat
	fprintf(fp,ZipBuffer);
	fclose(fp);
...
//	remove(Buffer);
	...
	else if (version != LTK_NODEVERSION) // fred ; Good for future compitability feeback
	{
		safe_bprintf(PRINT_MEDIUM, "ACE: Version is diffrent than this build\n");
        ACEIT_BuildItemNodeTable(false);
		return;
	}
	else
