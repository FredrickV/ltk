...
//FB 6/2/99 - shotgun/handcannon damage notification
        ...
        if (total_to_print)
        {
                ...
                safe_cprintf(self, PRINT_HIGH, "%s\n", textbuf);
        }
//FB 6/2/99
}
...
// 3.20 ANTI-LAG FIX, just in case we end up using this code for something... -FB
		// Paril: 3.21 added new SVF_PROJECTILE, but we don't use blaster!@
...
// ---
                                        ...
                                        && 1 ) // crandom() > .8 ) 
                                {
                                        ...
                                        if (IsFemale(tr.ent))
                                        {
                                                safe_cprintf(ent, PRINT_HIGH, "You kick %s's %s from her hands!\n", tr.ent->client->pers.netname, (tr.ent->client->pers.weapon)->pickup_name );
                                        }
                                        else
                                        {
                                                safe_cprintf(ent, PRINT_HIGH, "You kick %s's %s from his hands!\n", tr.ent->client->pers.netname, (tr.ent->client->pers.weapon)->pickup_name );
                                        }
                                        safe_cprintf(tr.ent, PRINT_HIGH, "%s kicked your weapon from your hands!\n", ent->client->pers.netname );
                                }
...
*/
        ...
        //      safe_cprintf(self, PRINT_HIGH, "speed %d\n", speed);
