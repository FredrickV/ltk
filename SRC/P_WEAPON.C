...
edict_t *FindSpecWeapSpawn( edict_t* ent )
{
        edict_t *spot = NULL;
                ...
                //safe_bprintf (PRINT_HIGH, "Calling the FindSpecWeapSpawn\n");
                spot = G_Find (spot, FOFS(classname), ent->classname);
                //safe_bprintf (PRINT_HIGH, "spot = %p and spot->think = %p and playerholder = %p, spot, (spot ? spot->think : 0), PlaceHolder\n");
        while ( spot && spot->think != PlaceHolder )//(spot->spawnflags & DROPPED_ITEM ) && spot->think != PlaceHolder )//spot->solid == SOLID_NOT )        
        {
                //              safe_bprintf (PRINT_HIGH, "Calling inside the loop FindSpecWeapSpawn\n");
                spot = G_Find (spot, FOFS(classname), ent->classname);
        }
/*      if (!spot)
        {
                safe_bprintf(PRINT_HIGH, "Failed to find a spawn spot for %s\n", ent->classname);
        }
        else
                safe_bprintf(PRINT_HIGH, "Found a spawn spot for %s\n", ent->classname);
*/
        return spot;
}
...
void Drop_Weapon (edict_t *ent, gitem_t *item)
{
        ...
        {
                safe_cprintf(ent, PRINT_HIGH, "You are too busy bandaging right now...\n");
                return;
        }
        ...
        {
                //safe_cprintf(ent, PRINT_HIGH, "Before checking knife inven frames = %d\n", ent->client->ps.gunframe);
                        ...
                        if ( ((ent->client->pers.weapon == item) ) && (ent->client->pers.inventory[index] == 1) )
                {
                        ...
                        if ( ent->client->resp.knife_mode ) // hack to avoid an error
                        {
                                ...
                                ChangeWeapon( ent );
                        //      safe_cprintf(ent, PRINT_HIGH, "After change weap from knife drop frames = %d\n", ent->client->ps.gunframe);
                }
        }
                        ...
                        case HC_NUM:
                                {
                                        ...
                                        // Paril - (Friday, May 12, 2006) 2:29 PM
										// Test, might look better
...
//FIREBLADE
                {
                        ...
                        case MK23_NUM:
                                {
                                        //      safe_cprintf (ent, PRINT_HIGH, "Calling ammo check %d\n", ent->client->mk23_rds);
                                        if ( ent->client->mk23_rds > 0 )
                                        {
                                                //              safe_cprintf(ent, PRINT_HIGH, "Entered fire selection\n");
                        ...
                        default:
                                {
                                        safe_cprintf (ent, PRINT_HIGH, "Calling non specific ammo code\n");
        ...
        if ( ent->client->weaponstate == WEAPON_BURSTING )
        {
                for (n = 0; fire_frames[n]; n++)
                {
                        if (ent->client->ps.gunframe == fire_frames[n])
                        {
                                if (ent->client->quad_framenum > level.framenum)                                                                gi.sound(ent, CHAN_ITEM, gi.soundindex("items/damage3.wav"), 1, ATTN_NORM, 0);
                                {
                //                      safe_cprintf (ent, PRINT_HIGH, "Calling fire code, frame = %d.\n", ent->client->ps.gunframe);
                                        fire (ent);
                                }
                                break;
                        }
                ...
                //safe_cprintf (ent, PRINT_HIGH, "Calling stricmp, frame = %d.\n", ent->client->ps.gunframe);
                ...
                //      safe_cprintf (ent, PRINT_HIGH, "Succes stricmp now: frame = %d.\n", ent->client->ps.gunframe);
...
//      safe_cprintf(ent, PRINT_HIGH, "Spread is %d\n", spread);
...
//FIREBLADE
        ...
        if (ent->client->weaponstate == WEAPON_ACTIVATING)
        {
                ...
                {
//                      safe_cprintf(ent, PRINT_HIGH, "NewKnifeFirst\n");
                ...
                ent->client->ps.gunframe++;
//              safe_cprintf(ent, PRINT_HIGH, "After increment frames = %d\n", ent->client->ps.gunframe);
                return;
        }
...
/* // zucc this causes you to not be able to throw a knife while it is flipping
                if ( ent->client->ps.gunframe == 93 )
                {
                        ...
                        return;
                                ...
                                */
                //safe_cprintf(ent, PRINT_HIGH, "Before a gunframe additon frames = %d\n", ent->client->ps.gunframe);
                ent->client->ps.gunframe++;
                return;
        }
        if (ent->client->weaponstate == WEAPON_FIRING  )
