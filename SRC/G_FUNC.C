...
void door_hit_top (edict_t *self)
{
        ...
        self->moveinfo.state = STATE_TOP;
		self->solid = SOLID_BSP;		//Werewolf: Added!
...
/* //Werewolf: Disabled this to keep bots from closing doors in front of their own path
        ...
        if (self->spawnflags & DOOR_TOGGLE)
        {
                ...
                {
//                        if ( other->client )
//                              safe_cprintf(other, PRINT_HIGH, "Client sending door down.\n");
...
*/
  //      if ( other->client )
//              safe_cprintf(other, PRINT_HIGH, "Client sending door up.\n");
...
{
        // zucc needed so the doors respond quickly
...
// zucc
        if ( other->client )
        {
                if (other->client->doortoggle == 0 ) // player not trying to open the door
                {
                        //safe_cprintf(other, PRINT_HIGH, "Client failed to open door.\n");
                        return;
                }
                else
                {
                        //other->client->doortoggle = 0;
...
void door_blocked  (edict_t *self, edict_t *other)
{
        ...
        if (!(other->svflags & SVF_MONSTER) && (!other->client) )
        {
                ...
                //zucc stop the doors from damaging people
        //T_Damage (other, self, self, vec3_origin, other->s.origin, vec3_origin, self->dmg, 1, 0, MOD_CRUSH);
...
// so let it just squash the object to death real fast
        ...
        if (self->moveinfo.wait >= 0)
        {
			//Werewolf: Door hack that makes doors passable for bots during their opening animation.
			if( (self->moveinfo.state != STATE_DOWN) && 
				((other->think == ACEAI_Think) || (other->think == BOTAI_Think)) )
				self->solid = SOLID_NOT;		//Werewolf: Added!
