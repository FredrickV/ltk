...
void grenadierThrow (edict_t *self);

mframe_t grenadier_frames_stand [] =
{
	ai_stand, 0, NULL,
	ai_stand, 0, NULL,
	ai_stand, 0, NULL,
...
mmove_t grenadier_move_stand = {0, 39, grenadier_frames_stand, NULL};

void grenadier_stand (edict_t *self)
{
	self->monsterinfo.currentmove = &grenadier_move_stand;
}


mframe_t grenadier_frames_fidget [] =
{
	ai_stand, 0, NULL,
	ai_stand, 0, NULL,
	ai_stand, 0, NULL,
...
mmove_t grenadier_move_fidget = {0, 39, grenadier_frames_fidget, grenadier_stand};

void grenadier_fidget (edict_t *self)
{
	self->monsterinfo.currentmove = &grenadier_move_fidget;
	//gi.sound (self, CHAN_VOICE, sound_idle, 1, ATTN_IDLE, 0);
}

mframe_t grenadier_frames_walk [] =
{
	ai_walk, 11,  NULL,
	ai_walk, 11,  NULL,
	ai_walk, 11,  NULL,
...
mmove_t grenadier_move_walk = {40, 45, grenadier_frames_walk, NULL};

void grenadier_walk (edict_t *self)
{
	self->monsterinfo.currentmove = &grenadier_move_walk;
}

mframe_t grenadier_frames_run [] =
{
	ai_run, 17, NULL,
	ai_run, 17, NULL,
	ai_run, 17, NULL,
...
mmove_t grenadier_move_run = {40, 45, grenadier_frames_run, NULL};

void grenadier_run (edict_t *self)
{
	if (self->monsterinfo.aiflags & AI_STAND_GROUND)
		self->monsterinfo.currentmove = &grenadier_move_stand;
	else
		self->monsterinfo.currentmove = &grenadier_move_run;
}


mframe_t grenadier_frames_pain1 [] =
{
	ai_move, 0, NULL,
	ai_move, 0, NULL,
	ai_move, 0, NULL,
...
mmove_t grenadier_move_pain1 = {54, 57, grenadier_frames_pain1, grenadier_run};

mframe_t grenadier_frames_pain2 [] =
{
	ai_move, 0, NULL,
	ai_move, 0,  NULL,
	ai_move, 0, NULL,
...
mmove_t grenadier_move_pain2 = {58, 61, grenadier_frames_pain2, grenadier_run};

mframe_t grenadier_frames_pain3 [] =
{
	ai_move, 0, NULL,
	ai_move, 0,  NULL,
	ai_move, 0, NULL,
...
mmove_t grenadier_move_pain3 = {62, 65, grenadier_frames_pain3, grenadier_run};

void grenadier_pain (edict_t *self, edict_t *other, float kick, int damage)
{
	int	n = rndnum (0, 2);
	int r = rndnum (0, 2);
	...
	if (n == 0)
	{
		self->monsterinfo.currentmove = &grenadier_move_pain1;
	}
	else if (n == 1)
	{
		self->monsterinfo.currentmove = &grenadier_move_pain2;
	}
	else
	{
		self->monsterinfo.currentmove = &grenadier_move_pain3;
	}
}
...
//	vec3_t	vec;
	...
	monster_fire_grenade (self, start, forward, GRENADE_DAMRAD, 720);
	gi.sound(self, CHAN_AUTO, gi.soundindex("weapons/HGRENT1A.WAV"), 1, ATTN_NORM, 0);
	self->pin = 0;
}

void grenadier_sight (edict_t *self, edict_t *other)
{
	//gi.sound (self, CHAN_BODY, sound_sight, 1, ATTN_NORM, 0);
}

void grenadier_dead (edict_t *self)
{
	VectorSet (self->mins, -16, -16, -24);
	VectorSet (self->maxs, 16, 16, -8);
	self->movetype = MOVETYPE_TOSS;
...
mframe_t grenadier_frames_death1 [] =
{
	ai_move, 0, NULL,
	ai_move, 0,  NULL,
	ai_move, 0,  NULL,
...
mmove_t grenadier_move_death1 = {178, 183, grenadier_frames_death1, grenadier_dead};

// Off with his head
mframe_t grenadier_frames_death2 [] =
{
	ai_move, 0,   NULL,
	ai_move, 1,   NULL,
	ai_move, 5,   NULL,
...
mmove_t grenadier_move_death2 = {184, 189, grenadier_frames_death2, grenadier_dead};

mframe_t grenadier_frames_death3 [] =
{
	ai_move, 0,   NULL,
	ai_move, 0,   NULL,
	ai_move, 0,   NULL,
...
mmove_t grenadier_move_death3 = {190, 197, grenadier_frames_death3, grenadier_dead};


void grenadier_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	int		n;
	int r = rndnum(0, 3);
...
// regular death
	...
	n = rand() % 3;
	if (n == 0)
		self->monsterinfo.currentmove = &grenadier_move_death1;
	else if (n == 1)
		self->monsterinfo.currentmove = &grenadier_move_death2;
	else
		self->monsterinfo.currentmove = &grenadier_move_death3;
}

void grenadier_ready_fire (edict_t *self)
{
	gi.dprintf ("Ready.,,\n");
}

void grenadier_fire (edict_t *self)
{
	if (!self->pin)
	{
		gi.sound(self, CHAN_AUTO, gi.soundindex("misc/grenade.wav"), 1, ATTN_NORM, 0);
		self->pin = 1;
		return;
	}
...
mframe_t grenadier_frames_attack1 [] =
{
	ai_charge, 0, grenadier_fire,
	ai_charge, 0, NULL,
	ai_charge, 0, NULL,
	ai_charge, 0, NULL,
	ai_charge, 0, NULL,
...
mmove_t grenadier_move_attack1 = {46, 53, grenadier_frames_attack1, grenadier_run};


/*void grenadier_swing (edict_t *self)
{
	gi.sound (self, CHAN_WEAPON, sound_punch_swing, 1, ATTN_NORM, 0);
}

void grenadier_smack (edict_t *self)
{
	vec3_t	aim;
...
mframe_t grenadier_frames_attack2 [] =
{
	ai_charge, 3, NULL,
	ai_charge, 6, NULL,
	ai_charge, 0, grenadier_swing,
	ai_charge, 8, NULL,
	ai_charge, 5, NULL,
	ai_charge, 8, grenadier_smack,
	ai_charge, 6, NULL,
	ai_charge, 3, NULL,
};
mmove_t grenadier_move_attack2 = {FRAME_attak201, FRAME_attak208, grenadier_frames_attack2, grenadier_run};*/

void grenadier_attack(edict_t *self)
{
	self->monsterinfo.currentmove = &grenadier_move_attack1;
}


/*QUAKED monster_mutant (1 .5 0) (-16 -16 -24) (16 16 32) Ambush Trigger_Spawn Sight
*/
void SP_monster_mutant (edict_t *self)
{
	if (deathmatch->value)
	...
	self->movetype = MOVETYPE_STEP;
	self->solid = SOLID_BBOX;
	self->s.modelindex = gi.modelindex("models/grenadier/tris.md2");
	self->s.modelindex2 = gi.modelindex("models/grenadier/a_m61frag.md2");
	VectorSet (self->mins, -16, -16, -24);
	...
	self->pain = grenadier_pain;
	self->die = grenadier_die;

	self->weap_num = 0;

	self->monsterinfo.stand = grenadier_stand;
	self->monsterinfo.walk = grenadier_walk;
	self->monsterinfo.run = grenadier_run;
	self->monsterinfo.dodge = /*grenadier_dodge*/NULL;
	self->monsterinfo.attack = grenadier_attack;
	self->monsterinfo.melee = NULL;
	self->monsterinfo.sight = grenadier_sight;
	self->monsterinfo.idle = grenadier_fidget;
