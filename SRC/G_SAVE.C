/*
 * $Header: /Action/LTK/SRC/G_SAVE.C,v 1.2 2007/03/09 03:13:36 Fredrick Exp $
 *
 * $Log: G_SAVE.C,v $
 * Revision 1.2  2007/03/09 03:13:36  Fredrick
 * *** empty log message ***
 *
 * Revision 1.1  2006/05/11 20:45:09  Fredrick
 * First Head
 *
 * 
...
field_t fields[] = {
		...
		// Paril, Additions for gamemodes.
		{"defend_target", FOFS(defend_target), F_INT},
		{"kill_target", FOFS(kill_target), F_INT},

		{"tempmode", FOFS(tempmode), F_INT},
...
//FIREBLADE
        ...
        if (coop->value)
        {
                ...
                gi.cvar_set("coop", "0");
        }*/ // Paril, Don't force deathmatch.

//FIREBLADE

        // change anytime vars
        dmflags = gi.cvar ("dmflags", "272", CVAR_SERVERINFO);
...
// WereWolf
 ...
 // Fred
        ...
        auto_bot  = gi.cvar("bots" , "0", CVAR_SERVERINFO); 
		gamemode = gi.cvar("gamemode" , "0", CVAR_SERVERINFO); 
		ltk_snipe = gi.cvar("ltk_snipe", "0", CVAR_SERVERINFO);
		ltk_trace = gi.cvar("trace","0",CVAR_SERVERINFO);
		debug_mode = gi.cvar("debug_mode","0",CVAR_SERVERINFO);
...
//ACEBOT END
		...
		g_sniperup = gi.cvar( "g_sniperup", "0", CVAR_USERINFO);

		mifed = gi.cvar ("mifed", "0", CVAR_LATCH|CVAR_SERVERINFO);
        ...
        flood_waitdelay = gi.cvar ("flood_waitdelay", "10", 0);
			...
			{
				gi.dprintf ("Teamplay and coop are enabled. Teamplay forced to 0.\n");
