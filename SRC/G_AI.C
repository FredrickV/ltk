...
edict_t *FindBadMonster (edict_t *self)
{	
	edict_t	*ent = NULL;	
	edict_t	*best = NULL;
	...
	{		
		...
		if (!visible(self, ent))			
			continue;
		if (ent->spawnflags & SPAWNFLAG_GOODGUY) // Both a good guy.
			continue;
...
edict_t *FindGoodMonster (edict_t *self)
{	
	edict_t	*ent = NULL;	
	edict_t	*best = NULL;
	...
	{		
		...
		if (!visible(self, ent))			
			continue;
		if (!(ent->spawnflags & SPAWNFLAG_GOODGUY) && !ent->client) // Both a bad guy.
			continue;
...
qboolean FindTarget (edict_t *self)
{
		...
		else if (!(self->spawnflags & SPAWNFLAG_GOODGUY))
		{
			monster = FindGoodMonster(self);		
			if (monster)		
			{
				self->enemy = monster;			
				FoundTarget (self);			
