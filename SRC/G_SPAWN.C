/*
 * $Header: /Action/LTK/SRC/G_SPAWN.C,v 1.2 2007/03/09 03:14:06 Fredrick Exp $
 *
 * $Log: G_SPAWN.C,v $
 * Revision 1.2  2007/03/09 03:14:06  Fredrick
 * *** empty log message ***
 *
 * Revision 1.3  2006/05/13 21:43:30  Fredrick
 * nothing special has changed
 *
 * Revision 1.2  2006/05/12 23:51:42  Fredrick
 * // == Modified by Paril == \\
 *
 * 1. Handcannon reload looks alot better now (check rounds when reloading)
 * 2. Bandaging now appears in HUD above-right of the ammo
 *
 * Revision 1.1  2006/05/11 20:45:11  Fredrick
 * First Head
 *
 * 
 * 6     2/11/99 12:58 Riever
 * Fog include file and call added.
 * 
 *
 */
...
//FIREBLADE
                                if ( item->flags == IT_AMMO || item->flags == IT_WEAPON )
                                        SpawnItem (ent, item);
								// Fix by Paril
								if (item->flags |= IT_KEY && coop->value)
									SpawnItem (ent, item);
                        }
...
//FIREBLADE
		...
		// Look through entities, might be a mistaken map!
		if (!deathmatch->value && level.coop_gamemode == 0)
		{
			gi.dprintf ("================================\n");
			gi.dprintf ("Map has no gamemode, checking ents for possible mismatch\n");
			gi.dprintf ("Searching for a Kill Target...\n");
			if (Coop_FindKillTarget() == true)
			{
				gi.dprintf ("Found one, setting mode!\n");
				gi.dprintf ("================================\n");
				goto finish;
			}
			gi.dprintf ("No Kill Target found!\n");
			gi.dprintf ("Searching for a Defend Target...\n");
			if (Coop_FindDefendTarget() == true)
			{
				gi.dprintf ("Found one, setting mode!\n");
				gi.dprintf ("================================\n");
				goto finish;
			}
			gi.dprintf ("No Defend Target found!\n");
			gi.dprintf ("Searching for a Capture Entity...\n");
			if (Coop_FindCaptureTarget() == true)
			{
				gi.dprintf ("Found one, setting mode!\n");
				gi.dprintf ("================================\n");
				goto finish;
			}
			gi.dprintf ("No Capture Entity found!\n");
			gi.dprintf ("Assuming that it's a stock map...\n");
			gi.dprintf ("================================\n");
		}
		else if (level.coop_gamemode == 1)
		{
			gi.dprintf ("================================\n");
			gi.dprintf ("Map has gamemode set to Extermination!\n");
			gi.dprintf ("================================\n");
		}
		else if (level.coop_gamemode == 2)
		{
			gi.dprintf ("================================\n");
			gi.dprintf ("Map has gamemode set to Kill Target!\n");
			gi.dprintf ("================================\n");
		}
		else if (level.coop_gamemode == 3)
		{
			gi.dprintf ("================================\n");
			gi.dprintf ("Map has gamemode set to Defend Target!\n");
			gi.dprintf ("================================\n");
		}
		else if (level.coop_gamemode == 4)
		{
			gi.dprintf ("================================\n");
			gi.dprintf ("Map has gamemode set to Capture!\n");
			gi.dprintf ("================================\n");
		}
...
"num 3 14"
*/
// Paril - (Friday, 12 May, 2006) 2:20 PM
// ammo
/*"if 30 "
"       xv      150 "
"       pic 30 "
...
void SP_worldspawn (edict_t *ent)
{
		...
		if (ent->tempmode)
			level.coop_gamemode = ent->tempmode;
...
//FIREBLADE
        ...
        if (teamplay->value)
        {
			// Paril: Fixed with graceful recovery
			// Todo: default to a skin.
                if (team1_skin_index[0] == 0)
                {
                        gi.error("No skin was specified for team 1 in config file.  Exiting.\nYou might be missing action.ini\n");
                        //exit(1);
                }
                gi.imageindex(team1_skin_index);
                if (team2_skin_index[0] == 0)
                {
                        /*gi.dprintf*/gi.error("No skin was specified for team 2 in config file.\nYou might be missing action.ini\n");
                        //exit(1);
                }
