/*
 * $Header: /Action/LTK/SRC/G_MAIN.C,v 1.2 2007/03/09 03:12:16 Fredrick Exp $
 *
 * $Log: G_MAIN.C,v $
 * Revision 1.2  2007/03/09 03:12:16  Fredrick
 * *** empty log message ***
 *
...
void G_RunFrame (void)
{
        ...
        // LTK
       if(level.time > 6.00) // oninit
	   {
		 if(level.AddTime == 0.00)
	       level.AddTime = level.AddTime + level.time+1.00;   
         if(level.time > level.AddTime)
			 if(auto_bot->value > level.bots)
			 {
				 ACESP_LoadBots();
				 level.bots++; level.AddTime +=1.00;
			 }
			 else
				 level.AddTime = -1.00;
	   }
