//-----------------------------------------------------------------------------
//
//  $Logfile:: /LicenseToKill/src/a_team.c                                    $
// $Revision: 1.1 $
//   $Author: Fredrick $
//     $Date: 2007/01/21 21:04:07 $
//
//-----------------------------------------------------------------------------
 ...
 *
 * 
 * 4     26/09/99 9:25 Riever
 * Changed string length safety checks in scoreboard to try to prevent
 * overflows.
 */
/*
...
void Team_f(edict_t *ent)
{
        ...
        if (!*t)
        {
                safe_cprintf(ent, PRINT_HIGH, "You are on %s.\n",
        ...
        {
                safe_cprintf(ent, PRINT_HIGH, "You must wait 5 seconds before changing teams again.\n");
                return;
        }
        ...
        {
                safe_cprintf(ent, PRINT_HIGH, "You are already on %s.\n",
                              TeamName(ent->client->resp.team));
                return;
        }
...
void UnevenTeamsMsg(int whichteam, int uneven_amount, char *opponent)
{
        ...
        for (i = 1; i <= maxclients->value; i++)
        {
                ...
                if (e->inuse)
                {
                        ...
                        {
                                safe_cprintf(e, PRINT_HIGH, "Your team now has %d more player%s than %s.\n", 
...
void LeaveTeam(edict_t *ent)
{
        ...
        if (IsNeutral(ent))
                ...
                g = "his";
        safe_bprintf(PRINT_HIGH, "%s left %s team.\n", ent->client->pers.netname, g);
...
void CenterPrintAll(char *msg)
{
        ...
        safe_cprintf(NULL, PRINT_HIGH, msg); // so it goes to the server console...
...
int WonGame(int winner)
{
        ...
        {
                safe_bprintf(PRINT_HIGH, "It was a tie, no points awarded!\n");
        }
                else        
        {
        ...
        if (roundlimit->value)
        {
                ...
                {
                        safe_bprintf(PRINT_HIGH, "Roundlimit hit.\n");
                        EndDMLevel();
                        team_round_going = team_round_countdown = team_game_going = 0;
                        return 1;
                }
...
void CheckTeamRules()
{
                ...
                else /* team_round_going */
        {
                ...
                {
                        safe_bprintf(PRINT_HIGH, "Round timelimit hit.\n");
...

        {
				...
				if (game.clients[sorted[i]].resp.damage_dealt < 1000000)
					...
					strcpy(damage, "******");
                        ...
                        if (strlen(string) > (maxsize - 200) && //RiEvEr was -100 
                                i < (total - 2))
                        {
                                sprintf(string + strlen(string),
