...
void PrecacheRadioSounds()
{
   ...
   //     gi.soundindex(RADIO_DEATH_MALE);  We dont want to play the rdeath.wav sound :) Fred
...
{
	//LTK RADIO
	// Bots can't receive radio messages...
	if(ent->is_bot)
		return;
	//LTK
        if (ent->client->resp.radio_queue_size == 0 ||
...
void RadioBroadcast(edict_t *ent, int partner, char *msg)
{
        ...
        while (strcmp(radio_msgs[i].msg, "END"))
        {
                ...
                {
					//RiEvEr LTK Bot support
					if( !Q_stricmp("treport", msg))
					{
						safe_bprintf(PRINT_HIGH,"Requested Team Report In.\n");
						// Get bots to report in
						bBotReport = true;
					}
                    found = 1;
        ...
        for (j = 1; j <= game.maxclients; j++)
        {
                ...
                if (partner && other != ent->client->resp.radio_partner)
                        continue;
				if( other->is_bot && bBotReport)
				{
					safe_bprintf(PRINT_HIGH,"Ordered to report in.\n");
					// Signal that we should report in
					other->teamReportedIn = false;
					other->teamReportin = false; //Fred
					// Make sure there's a delay
					other->lastRadioTime = level.time + random()*3.0;
				}
