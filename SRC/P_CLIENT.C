//-----------------------------------------------------------------------------
//
//  $Logfile:: /LTK2/src/p_client.c                                           $
// $Revision: 1.2 $
//   $Author: Fredrick $
//     $Date: 2007/03/09 03:15:39 $
//
// Modifications Copyright (C) 1999 by Connor Caple
// Original code Copyright (c) 1998 ID Software
// All rights reserved.
//
//-----------------------------------------------------------------------------
...
/*
 * $Log: P_CLIENT.C,v $
 * Revision 1.2  2007/03/09 03:15:39  Fredrick
 * *** empty log message ***
 *
 * Revision 1.2  2006/05/13 21:44:54  Fredrick
 * fixed a bug with the grenade
 *
 * Revision 1.1  2006/05/11 20:45:10  Fredrick
 * First Head
 *
 * 
 * 3     23/02/00 17:24 Riever
 * Added support for 'sv shownodes on/off'
 * Enabled creation of nodes for ALL doors. (Stage 1 of new method)
 * 
 * 2     17/02/00 17:10 Riever
 * Radiogender now set for bots.
 * 
 * 13    16/02/00 17:58 Riever
 * Added jump checks with is_jumping to fix jump node creation.
 * 
 * 12    16/02/00 16:27 Riever
 ...
 * 
 * 11    22/10/99 20:24 Riever
 * Tidying up...
 * 
 * 10    16/10/99 10:27 Riever
 * Stopped bots dropping nodes - too many bad links.
 * 
 * 9     16/10/99 9:14 Riever
 * Changed to load new config info.
 * 
 * 8     6/10/99 20:20 Riever
 * Re-enabled bots.tmp loading.
 * 
 * 7     5/10/99 20:09 Riever
 * Changes to let bots load the route file and load the bots in a
 * dedicated server after the level has changed.
 * 
 * 6     29/09/99 8:45 Riever
 * lastkilledby is now used to insult the person who killed the bot.
 * 
 * 5     27/09/99 20:45 Riever
 * Allowed bots to create nodes like players do.
 * 
 * 4     26/09/99 8:14 Riever
 * Added to logging for LTK
 * 
 */
...
void Add_Frag( edict_t *ent )
{
        ...
        {
                safe_bprintf (PRINT_MEDIUM,"%s has %d kills in a row and receives %d frags for the kill!\n",
...
//FIREBLADE
        {
                safe_cprintf(ent, PRINT_HIGH, "Kill count: %d\n", ent->client->resp.streak );
        }
}
...
void Add_TeamWound( edict_t *attacker, edict_t *victim, int mod)
{
	...
	if (attacker->client->ff_warning == 0)
	{
		...
		safe_cprintf(victim, PRINT_HIGH, "You were hit by %s, your TEAMMATE!\n", 
				attacker->client->pers.netname);
		safe_cprintf(attacker, PRINT_HIGH, "You hit your TEAMMATE %s!\n", 
	...
	{		
		...
		safe_cprintf(NULL, PRINT_MEDIUM, 
			"%s is in danger of being banned for wounding teammates\n",
				attacker->client->pers.netname);
		safe_cprintf(attacker, PRINT_HIGH, 
...
void Add_TeamKill( edict_t *attacker )
{
	...
	{
		safe_cprintf(attacker, PRINT_HIGH, "You killed your TEAMMATE!\n");
		return;
	}
	else if (attacker->client->team_kills < (int)maxteamkills->value)
	{
		// Show this on the console
		safe_cprintf(NULL, PRINT_MEDIUM, "%s is in danger of being banned for killing teammates\n", attacker->client->pers.netname);
		// Issue a warning to the player
		safe_cprintf(attacker, PRINT_HIGH, "WARNING: You'll be temporarily banned if you continue killing teammates!\n" );
		return;
	}
	else
	{
		...
		if (attacker->client->ipaddr)
		{
			if (Ban_TeamKiller(attacker, (int)tkbanrounds->value))
			{
				...
				safe_cprintf(attacker, PRINT_HIGH, "You've killed too many teammates, and are banned for %d %s.\n", (int)tkbanrounds->value,
...
static void SP_CreateSpSpots (edict_t *self)
{
        edict_t *spot;

		
        if(stricmp(level.mapname, "lostcoast") == 0)
        {
                spot = G_Spawn();
                spot->classname = "info_player_spawn";
                spot->s.origin[0] = 188 - 64;
                spot->s.origin[1] = -164;
                spot->s.origin[2] = 80;
                spot->targetname = "lostcoast";
                spot->s.angles[1] = 90;
		}
}
...
qboolean IsFemale (edict_t *ent)
{
        ...
        if (info[0] == 'f' || info[0] == 'F')
                        return true;
		//RiEvEr LTK - Get bot gender
		else if (ent->is_bot)
		{
			info = Info_ValueForKey (ent->client->pers.userinfo, "skin");
			// Currently on the female/skin set begins with 'f'
			if (info[0] == 'f' || info[0] == 'F')
							return true;
			if (info[0] == 's' || info[0] == 's')
				if (info[0] == 'y' || info[0] == 'Y')	// Sydney!!
					return true;
		}
		//R LTK
        return false;
...
char *FindMonsterName (edict_t *attacker)
{
//	char *name;
	// Paril, finish!
	return "a monster";
...
void Monster_Obituary (edict_t *self, edict_t *inflictor, edict_t *attacker, int mod)
{
	char *message;
	char *message2 = "";
	char *monstername = FindMonsterName (attacker);
	int n; 
	short int def =1;
	...
	default:{
        ...
        if(def)
		  safe_bprintf (PRINT_MEDIUM, "%s%s %s%s\n", self->client->pers.netname, message, monstername, message2); 
}
...
void ClientObituary (edict_t *self, edict_t *inflictor, edict_t *attacker)
{
	...
	//R
		...
		// Paril, Was it a monster that did the killing?
		if (attacker && !attacker->client && attacker != world && mod != MOD_UNKNOWN)
		{
			Monster_Obituary (self, inflictor, attacker, mod);
			return;
		}
		// Paril
...
//FIREBLADE
        ...
        self->s.modelindex4 = 0; // Remove briefcase
...
void SaveClientData (void)
{
        ...
        for (i=0 ; i<game.maxclients ; i++)
        {
                ...
                if (!ent->inuse)
                        continue;
                game.clients[i].pers.health = 100/*ent->health*/; //fred - Aq2 coop is to hard, so lets give them some health on new coop map
...
void EquipClient( edict_t *ent )
{
        ...
        else if ( stricmp(client->resp.weapon->pickup_name, SNIPER_NAME ) == 0 )
        {
			...
			int value;
				...
				value = atoi(Info_ValueForKey (userinfo, "g_sniperup"));
                item = FindItem(SNIPER_NAME);
				// Paril
				// A client-side variable saved in userinfo, allows
				// them to choose whether the sniper is taken up first
				// or the mk23. Defaulted to mk23.
				if (value)
				{
					client->pers.selected_item = ITEM_INDEX(item);
					client->pers.inventory[ITEM_INDEX(item)] = 1;
...
//FIREBLADE
        ...
        //item = FindItem("Pistol Clip");     
        // Add_Ammo(ent,item,1);
		// Changed the client, to force them to follow the gamemodes rules Fred
{
		if (gamemode->value != 0)
		{
       client->mk23_max = 0;
...
//FIREBLADE
// ACEBOT_ADD
		safe_centerprintf(ent,"\n======================================\nL.T.K. 2.0 Beta April 1 2007 "
			"AQ2 Mod\n\n'sv addbot' to add a new bot.\n'sv removebot <name>' to remove bot.\n\nsv gamemode(0-6) "
			":\n0 = Default (random) mode\n1 = Mp5 mode\n2 = M4 mode\n3 = M3 mode \n4 = HC mode \n 5 = SSG "
			"mode \n6 = Knife War mode(KnifeWar is the only\nexperimental gamemode,\na restart is required to cycle\n"
			"the other (0-5) game modes\nnormaly again.)\n\n======================================\n\n");
...
//EEK
	...
	/* RiEvER ACEBOT - commented out for debugging
//FIREBLADE
...
// zucc stumbling associated with leg damage
                ...
                if ( ent->client->leg_damage )
                {
                        ...
                        {
                        //      safe_cprintf(ent, PRINT_HIGH, "Cutting velocity\n");
                                pm.s.velocity[2] = 0.0;
                        }
                }
                ...
                {
//                      safe_cprintf(ent, PRINT_HIGH, "Playing sound?\n");
...
// ACEBOT_ADD
	if (/*!ent->is_bot &&*/ !ent->deadflag && 
		!(ent->solid == SOLID_NOT) && ltk_routing->value)
		ACEND_PathMap(ent);
// ACEBOT_END

	//RiEvEr - New node visibility method
	if( !ent->is_bot && debug_mode->value && shownodes_mode )
		BOTUT_ShowNodes( ent );
	//R
