...
void barrel_explode (edict_t *self)
{
        ...
        T_RadiusDamage (self, self->activator, self->dmg, NULL, self->dmg+200, MOD_BARREL);

        VectorCopy (self->s.origin, save);
        VectorMA (self->absmin, 0.5, self->size, self->s.origin);

        // a few big chunks
        spd = 2.5 * (float)self->dmg / 100.0;
...
{
// zucc modified to remove visible teleporter/dm start
        //gi.setmodel (ent, "models/objects/dmspot/tris.md2");
        ent->s.skinnum = 0;
        ent->solid = SOLID_NOT;
        //ent->solid = SOLID_BBOX;
